import { PrismaClient, User, Company, UserStock, Stock, BuyOffer, SellOffer } from "@prisma/client";
...
for (let i = 1; i <= amountOfUsers; i++) {
  ...
  for(let j = 1; j <= amountOfUserStocks; j++)
  {
    ...
    const userCompanyId = faker.datatype.number({min:1, max:amountOfCompanies});
...
for(let i = 1; i<= amountOfBuyOffers; i++) {
  ...
  const userId = faker.datatype.number({min:1, max:amountOfUsers});
  const userStockId = faker.datatype.number({min:1, max:amountOfStockOffers});
  const userUnitBuyPrice = faker.datatype.number({min:1, max:maxOfferCents});
...
for(let i = 1; i<= amountOfSellOffers; i++) {
  ...
  const userId = faker.datatype.number({min:1, max:amountOfUsers});
  const userStockId = faker.datatype.number({min:1, max:userStockIdCounter});
  const userUnitSellPriceCents = faker.datatype.number({min:1, max: maxOfferCents});
